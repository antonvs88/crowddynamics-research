...
class Configuration:
    ...
    def set_body(self, size, body):
        ...
        # Agent class
        self.agent = Agent(strategy, tau_adj, A_agent, B_agent, A_wall, B_wall, self.walls.size, size, mass, radius,
                           ratio_rt, ratio_rs, ratio_ts, inertia_rot, target_velocity, target_angular_velocity)
...
class MultiAgentSimulation(Process, Configuration):
	...
	# Agents don't actually play a game in these simulations. Agent's are given fixed
        # strategies so that we can use the same crowd movement simulator as used in the
        # folder "model_with_game"
        self.game.strategy = self.agent.strategy
	...
	# Indices of agents that are playing the game
        ...
        mask = self.agent.active & self.agent.in_room1
        indices = np.arange(self.agent.size)[mask]

        # Agents that are not playing anymore will be patient again
        self.game.strategy[mask ^ True] = 1
