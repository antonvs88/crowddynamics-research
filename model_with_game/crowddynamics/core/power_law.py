...
def force_social_circular(agent, i, j):
    ...
    grad = gradient_circle_circle(x_rel, v_rel, a, b, d)
    force[0][:] += -agent.k_soc[i] * mag * grad
    force[1][:] -= -agent.k_soc[j] * mag *grad
