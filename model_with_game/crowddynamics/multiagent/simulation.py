...
from crowddynamics.core.vector2D import angle, length, length_nx2
...
class Configuration:
    ...
    def set_field(self, domain=None, obstacles=None, exits=None):
        ...
        *exits*     Collection of polygons and LineStrings.
        ...
        self.omega = Path(np.asarray(self.room1.exterior))
    ...
    def set_body(self, size, body):
        ...
        # Agent class
        self.agent = Agent(self.walls.size, size, mass, radius, ratio_rt, ratio_rs, ratio_ts,
    ...
    def set_agents(self, size=None, surface=None, position="random",
        ...
        *orientation*      float: Custom value  \n
        ...
        # IMPORTANT: the input for PolygonSample is the first room
...
class MultiAgentSimulation(Process, Configuration):
    ...
    def __init__(self, queue: Queue = None):
        ...
        # Integration step for Velocity Verlet
        self.dt = 0.001

        self.stationary_regime = 200
    ...
    def run(self):
        ...
        while (not self.exit.is_set()) and self.in_goal < self.stationary_regime:
            self.update()
    ...
    def update(self):
        ...
        # Update motion related parameters in Helbing's model
        # When self.game.strategy == 0, agent is impatient, and when self.game.strategy == 1 agent is patient

        self.agent.target_velocity[np.where(self.game.strategy == 0)[0]] = 5
        self.agent.target_velocity[np.where(self.game.strategy == 1)[0]] = 1

        self.agent.tau_adj[np.where(self.game.strategy == 0)[0]] = 0.5
        self.agent.tau_adj[np.where(self.game.strategy == 1)[0]] = 0.5

        self.agent.A_agent[np.where(self.game.strategy == 0)[0]] = 1000
        self.agent.A_agent[np.where(self.game.strategy == 1)[0]] = 2000

        self.agent.B_agent[np.where(self.game.strategy == 0)[0]] = 0.08
        self.agent.B_agent[np.where(self.game.strategy == 1)[0]] = 0.08

        self.agent.A_wall[np.where(self.game.strategy == 0)[0]] = 2000
        self.agent.A_wall[np.where(self.game.strategy == 1)[0]] = 2000

        self.agent.B_wall[np.where(self.game.strategy == 0)[0]] = 0.08
        self.agent.B_wall[np.where(self.game.strategy == 1)[0]] = 0.08

        i = self.agent.indices()
        # (1) Calculate force F(t) (this needs to be calculated only on the first iteration, after that F from step (4) is used)
        if self.iterations == 0:
            self.navigation.update() # path finding
            force_adjust(self.agent)  # adjusting force
            force_fluctuation(self.agent)
            agent_wall(self.agent, self.walls) # interaction forces between agents and walls
            agent_agent(self.agent)  # interaction forces between agents

        # (2) Calculate half-step velocity v(t+dt/2) = v(t) + 0.5*sum(F(t))/m*dt
        self.agent.velocity[i] += 0.5 * self.agent.force[i] / self.agent.mass[i] * self.dt

        # (3) Calculate position x(t+dt) = x(t) + v(t+dt/2)*dt
        self.agent.position[i] += self.agent.velocity[i] * self.dt

        # Reset
        self.agent.reset_motion()
        self.agent.reset_neighbor()

        # (4) Calculate force F(t+dt) using force_adjust, agent_agent, agent_wall
        self.navigation.update()  # path finding
        force_adjust(self.agent)  # adjusting force
        force_fluctuation(self.agent)
        agent_agent(self.agent)  # interaction forces between agents
        ...
        # (5) Calculate v(t+dt) = v(t+dt/2) + 0.5*sum(F(t+dt))/m*dt
        self.agent.velocity[i] += 0.5 * self.agent.force[i] / self.agent.mass[i] * self.dt

        # Check which agents are inside the domain
        if self.domain is not None:
            num = np.sum(self.agent.in_room1)
            self.agent.in_room1 &= self.omega.contains_points(self.agent.position)
            num -= np.sum(self.agent.in_room1)
            self.in_goal += num
        ...
        if self.hdfstore is not None:
            ...
            if (self.iterations % 100 == 0) or (self.in_goal == 200):
