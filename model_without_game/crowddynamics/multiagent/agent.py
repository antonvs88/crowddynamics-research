...
class Agent(object):
    ...
    def __init__(self, strategy, tau_adj, A_agent, B_agent, A_wall, B_wall, walls, size, mass, radius, ratio_rt,
                 ratio_rs, ratio_ts, inertia_rot, target_velocity, target_angular_velocity):
        ...
        self.tau_adj = tau_adj  # Adjusting force
        ...
        self.mu = 1.2e5  # Body force counteracting body compression. 12e4 in FDS+Evac
        self.kappa = 2.4e5 # Tangential friction force. 4e4 in FDS+Evac
        self.damping = 0  # Damping force. 500 in FDS+Evac
        self.std_rand_force = np.zeros(self.size)  # Fluctuation force
